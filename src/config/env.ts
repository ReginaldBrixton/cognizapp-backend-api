...
// Do not override existing process.env: tests and child processes set vars
// (e.g. PORT, ENVIRONMENT) that must win over values in .env.
...
assertCognizAppDatabase("DATABASE_URL", env.databaseUrl);
assertCognizAppDatabase("DATABASE_URL_DEV", env.databaseUrlDev);
assertCognizAppDatabase("DATABASE_URL_PROD", env.databaseUrlProd);

if (env.jwtSecret.length < 32) {
  throw new Error("JWT_SECRET must be at least 32 characters long");
}

if (env.devAuthEndpointEnabled) {
  if (!env.isDevelopment) {
    throw new Error("DEV_AUTH_ENDPOINT_ENABLED can only be used when ENVIRONMENT=development");
  }

  if (env.devAuthEndpointSecret.length < 48) {
    throw new Error(
